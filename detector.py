...
class OwlDetectorbase16:
    def __init__(self, model_name: str = "google/owlvit-base-patch16", device: str | None = None):
        if device is None:
            device = "cuda" if torch.cuda.is_available() else "cpu"
        self.device = device
        self.processor = OwlViTProcessor.from_pretrained(model_name)
...
class OwlDetectorlarge14:
    def __init__(self, model_name: str = "google/owlvit-large-patch14", device: str | None = None):
        if device is None:
            device = "cuda" if torch.cuda.is_available() else "cpu"
        self.device = device
        self.processor = OwlViTProcessor.from_pretrained(model_name)
