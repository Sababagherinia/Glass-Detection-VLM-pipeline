...
This repository contains a minimal skeleton pipeline to read RGB-D frames, process them with a visual-language model (e.g CLIP via Hugging Face `transformers`), and accumulate a 3D map (Open3D output, optional pyoctomap integration).
...
# If you want pyoctomap (optional), install it separately (may require system libs)
# pip install pyoctomap
```

2. Prepare folder-mode data:

Create a folder `data` with subfolders `rgb` and `depth`. Place matching RGB images in `data/rgb` and depth images in `data/depth` (16-bit PNG in millimeters or float meters accepted).

3. Run the pipeline (folder playback):

```powershell
python vln-drone-pipline.py --mode folder --data-dir data --out output --frames 100
```
